...
## $(date +%Y-%m-%d) - [O(1) Player Lookups in Systems and UI]
**Learning:** `engine.getActiveEntities().find { it.tag == "player" }` and `entities.find { it.tag == "player" }` calls scattered across abilities, UI components (`GameRenderer`, `MinimapComposable`), and view models allocate an `Iterator` and lambda closure each frame while performing an O(N) search.
**Action:** Replace all such lookups with `engine.playerEntity` to achieve O(1) time complexity and zero GC allocations. When in Compose functions, ensure `engine.playerEntity` is accessed directly from the `engine` instance instead of iterating the provided active entities list.
## 2023-10-27 - Active Entity Caching & O(1) Lookups
**Learning:** `getActiveEntities()` returned a new list or lambda operations like `.find {}` and `.filter {}` were extensively used on the active entities list per-frame in `GameViewModel`, `StageHazardSystem`, `GameModes`, etc. This allocated new iterator instances and closure wrappers every tick, causing GC spikes.
**Action:** Replaced `.find {}` and `.filter {}` with indexed `for` loops against `cachedActiveEntities` array. Also identified that `StageHazardSystem` searched for an entity by ID linearly; swapped it to the O(1) lookup `engine.getEntityById(id)`. When reviewing ECS query performance, check for standard Kotlin lambda functions in systems and replace them with indexed loops and engine lookups.
## $(date +%Y-%m-%d) - Distance-based early exits for Projectiles and Spatial Queries
**Learning:** `SpatialGrid.queryRange` searches for enemies and iterates through cell buckets. For projectiles and poison clouds far off-screen (> 1000 units), these queries are guaranteed to find nothing since enemies are actively culled beyond 900 units by the `WaveManagerSystem`, meaning this was wasting CPU cycles in `CollisionSystem.kt`. Furthermore, fast-moving projectiles were lingering in memory and the object pool until their lifetime expired (often several seconds), even when they had traveled thousands of units off-screen and were no longer relevant.
**Action:** Always cross-reference despawn boundaries across systems. In `CollisionSystem`, skip `SpatialGrid.queryRange` queries entirely if the querying entity is farther than the maximum enemy despawn distance. In `ProjectileSystem`, aggressively recycle projectiles (`active = false`) once they exit the maximum possible gameplay bounding box (>1200 units) to free up slots in the `ObjectPool` immediately instead of waiting for their lifetime timer.
