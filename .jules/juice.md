...
## 2026-10-27 - Progress Bar Extraction and Consistency
**Learning:** Hardcoded progress bars using multiple nested `Box` elements and customized styling were scattered across `GameHud.kt` and `TutorialScreen.kt`. While they looked identical visually, repeating the logic increased the risk of styling drift (where one gets updated but not the other) and reduced maintainability. In addition, when extracting UI elements that carry crucial visual and textual context, it is important to provide a `content` slot (e.g. `content: @Composable BoxScope.() -> Unit`) so caller sites can easily place text inside the shared component without breaking layout logic.
**Action:** Created the shared `HordeProgressBar` in `HordeUI.kt` which abstracts the standard `SmallCutShape` clipping, nested progress width adjustment via `.fillMaxWidth(progress.coerceIn(0f, 1f))`, and custom coloring. Replaced all occurrences in the app with this shared widget. Always extract visually similar complex layouts into highly configurable shared components instead of duplicating `Box` combinations.
